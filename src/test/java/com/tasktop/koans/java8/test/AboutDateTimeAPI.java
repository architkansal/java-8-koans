...
public class AboutDateTimeAPI {
	...
	public void java8_datesAreImmutable() {
		...
		LocalDateTime date2 = date.plusMinutes(durationtoAdd.toMinutes()); // See if you can find a way to add the durationToAdd to the date
	...
	public void java8_getNowWithSpecificClock() {
		Clock clock = Clock.fixed(Instant.EPOCH, ZoneId.systemDefault()); // TODO: create a clock that start at the epoch and uses the sytem timezone
	...
	public void java8_durationsCanAlsoBeUsedToCreateDate() {
		...
		LocalDateTime future = now.plusDays(duration.toDays()); // add the duration to now 
	...
	public void java8_adjustDatesBySemantic() {
		...
		LocalDate saturday = date.with(TemporalAdjusters.next(DayOfWeek.SATURDAY)); // find out how to get the next Saturday.
	...
	public void java8_useFixedOffsetsForTimeZones() {
		...
		OffsetDateTime date = OffsetDateTime.of(2000, 11, 23, 8, 14, 14, 14, ZoneOffset.ofHours(-1));
	...
	public void java8_useZoneIdsForTimeZones() {
		ZonedDateTime berlin = ZonedDateTime.now(ZoneId.of("Europe/Berlin")); // How do we find out the ZoneID of Berlin?
